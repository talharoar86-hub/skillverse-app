...
import { Search, MessageSquare, GraduationCap, BookOpen, Repeat } from 'lucide-react';
...
const ROLE_CONFIG = {
  Mentor: { goal: 'Mentor', icon: GraduationCap, emptyTitle: 'No mentor chats yet', emptyDesc: 'Start a conversation with a mentor from the Discover page.' },
  Learner: { goal: 'Learn', icon: BookOpen, emptyTitle: 'No learner chats yet', emptyDesc: 'Start a conversation with a learner from the Discover page.' },
  Exchanger: { goal: 'Exchange', icon: Repeat, emptyTitle: 'No exchanger chats yet', emptyDesc: 'Start a conversation with an exchanger from the Discover page.' },
};

const FilteredConversationList = ({ roleFilter }) => {
  ...
  const config = ROLE_CONFIG[roleFilter];
  const goalValue = config?.goal;
  ...
  const filteredConversations = conversations
    .filter(conv => {
      if (!goalValue) return true;
      return conv.otherUser?.goal === goalValue;
    })
    .filter(conv => {
  ...
  return (
    <div className="max-w-[800px] mx-auto">
      <div className="mb-6">
        <h1 className="text-2xl font-black text-slate-900 tracking-tight">{roleFilter}s</h1>
        <p className="text-sm text-slate-500 font-medium mt-1">
          Conversations with {roleFilter.toLowerCase()}s
        </p>
      </div>
      ...
      <div className="relative mb-4">
        ...
        <input
          type="text"
          placeholder={`Search ${roleFilter.toLowerCase()} chats...`}
          value={searchQuery}
      ...
      {filteredConversations.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-20 text-center">
          ...
          <h3 className="text-lg font-black text-slate-800 mb-2">{config?.emptyTitle || 'No conversations yet'}</h3>
          <p className="text-sm text-slate-500 font-medium max-w-xs">
            {config?.emptyDesc || 'Start chatting to see conversations here.'}
          </p>
        </div>
      ) : (
