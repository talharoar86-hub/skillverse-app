...
const ExchangersDiscoveryPage = () => {
  ...
  const fetchUsers = async () => {
    try {
      const data = await profileService.getDiscovery('Exchange');
      setUsers(data);
    } catch (err) {
  ...
  return (
    <div className="max-w-[800px] mx-auto">
      <div className="mb-6">
        <h1 className="text-2xl font-black text-slate-900 tracking-tight">Exchangers</h1>
        <p className="text-sm text-slate-500 font-medium mt-1">
          {users.length} {users.length === 1 ? 'exchanger' : 'exchangers'} available
        </p>
      </div>
      ...
      {users.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-20 text-center">
          ...
          <h3 className="text-lg font-black text-slate-800 mb-2">No exchangers found</h3>
          <p className="text-sm text-slate-500 font-medium max-w-xs">
            Exchangers will appear here once they set their goal to Exchange.
          </p>
        </div>
      ) : (
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-[11px] text-slate-400 font-medium truncate">
                  {user.experienceLevel || 'Exchanger'}
                </p>
              </div>
...
export default ExchangersDiscoveryPage;
